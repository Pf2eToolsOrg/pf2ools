import Document from "$lib/Data/DataUtils";
import { heritages } from "Storage";

export class Ancestry extends Document {
	constructor(data) {
		super(data);

		heritages.subscribe((heritages) => {
			this.heritages = heritages.filter(
				(h) => h.versatile || (h.ancestryName === this.name && h.ancestrySource === this.source)
			);
		});
		...
		Object.defineProperty(this, "images", {
			get() {
				...
				if (this?.selectedHeritage?.images)
					array.push({
						type: "pf2-h3",
						name: this.selectedHeritage.name,
						entries: [{ type: "hr" }, ...this.selectedHeritage.images],
					});
				return array;
			},
		});
...
export class Heritage extends Document {
	...
	tag(displayText, ancestryName, ancestrySource) {
		return `${ancestryName ?? this.ancestryName ?? "Human"}|${ancestrySource ?? this.ancestrySource ?? "CRB"}|${
