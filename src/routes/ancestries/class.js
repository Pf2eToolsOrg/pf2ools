...
export default class Ancestry extends DataEntry {
    ...
    get extraHp() {
        ...
        this.heritage.forEach((h) => {
            if (h?.modif?.hp) {
                array.push(({ hp: h.modif.hp, name: h.name, source: h.source, hash: this.hashify(h.name, h.source) }))
            }
        });
...
export class Heritage extends DataEntry {
    ...
    tag(displayText) {
        return `${this.ancestry.name}|${this.ancestry.source}|${displayText ? displayText : this.name}|${this.shortName}|${this.source}`
    }
}